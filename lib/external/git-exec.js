...
function git_class_exec(command, repo, where, callback) {
  return git_class_exec_advanced(command, undefined, repo, where, callback);
}

function git_class_exec_advanced(command, command_args, repo, where, callback) {
  var args = [];

  // add the command args
  if (command_args) { args.push(command_args); }
...
Git.shallowCopyClone = function(repo, dir, callback) {
    if (!dir) {
        callback(null);
        return;
    }

    git_class_exec_advanced('clone', '--depth 1', repo, dir, callback);
};
